...
from auth import (
    ...
    get_current_admin, handle_google_web_callback, handle_google_mobile_callback, oauth
)
import uuid
...
app = FastAPI(title="StudyPal RAG API", description="Unified API for study assistant and user management.")
...
async def google_mobile_login(id_token: str = Form(...), db: Session = Depends(get_db)):
    return await handle_google_mobile_signin(id_token, db)
...
@app.post("/auth/google/mobile", tags=["Authentication"])
async def google_mobile_login(request: Request, db: Session = Depends(get_db)):
    return await handle_google_mobile_callback(request, db)
